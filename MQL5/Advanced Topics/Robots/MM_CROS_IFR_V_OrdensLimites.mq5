    ...
    if(temosNovaVela)
      {
       ...
       // Condição de Compra:
       if(Comprar && OrderSelect(OrderGetTicket(0))==false )
         {
          desenhaLinhaVertical("Compra",velas[1].time,clrBlue);
          //CompraAMercado();
          CompraLimite( NormalizeDouble(velas[1].low - TK*_Point,_Digits) ,TK,SL);
         }
       
       // Condição de Venda:
       if(Vender && !OrderSelect(OrderGetTicket(0)) )
         {
          desenhaLinhaVertical("Venda",velas[1].time,clrRed);
          //VendaAMercado();
          VendaLimite( NormalizeDouble(velas[1].high + TK*_Point,_Digits) ,TK,SL);
          
         } 
         
      }
      
      if(TimeToString(TimeCurrent(),TIME_MINUTES) == "17:30" && OrderSelect(OrderGetTicket(0)) )
        {
...
void CompraLimite(double nivel_compra, double tp = 0.0, double sl = 0.0)
   {
      ...
      trade.BuyLimit(num_lots,nivel_compra,_Symbol, NormalizeDouble(nivel_compra - sl*_Point,_Digits), 
...
void VendaLimite(double nivel_venda, double tp = 0.0, double sl = 0.0)
   {
      ...
      trade.SellLimit(num_lots,nivel_venda,_Symbol, NormalizeDouble(nivel_venda + sl*_Point,_Digits),
