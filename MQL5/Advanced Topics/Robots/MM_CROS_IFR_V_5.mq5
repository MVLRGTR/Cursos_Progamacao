...
// Estratégia baseada em SL móvel min do candle anterior!!
...
input LIGA   liga_sl_movel                 = SIM;           // Liga Stop Loss Móvel
input double pts_sl                        = 30;           // Pts Extra Stop Loss Móvel
       ...
       if( liga_breakeven == SIM && PositionSelect(_Symbol) && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
         {
            ...
            Print("Preço da metade de uma Compra = ", preco_entrada);
       ...
       if( liga_breakeven == SIM && PositionSelect(_Symbol) && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL)
         {
            ...
            desenhaLinhaHorizontal("Venda",preco_medio,clrYellow);
            ...
            Print("Preço da metade de uma Venda = ", preco_entrada);
            ...
            if( tick.last > novo_sl && preco_sl != novo_sl && novo_sl > preco_sl) // 
              {
                  ...
                  //trade.PositionClosePartial(PositionGetTicket(0),num_lots/2);
              }
        ...
        if(liga_sl_movel == SIM && PositionSelect(_Symbol) && PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL )
          {
                  ...
                  double novo_sl       = NormalizeDouble(velas[1].high + pts_sl*_Point,_Digits);
