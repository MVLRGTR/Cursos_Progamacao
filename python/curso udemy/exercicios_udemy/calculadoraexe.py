...
def raiz_quadrada(valor):
    ...
    precisao = 0.000000000001
    while True:
        prox_estimativa = ((2-1) * estimativa + valor / (estimativa**(2-1))) / 2
        if abs(prox_estimativa - estimativa) < precisao:
            return prox_estimativa
        estimativa = prox_estimativa
...
def fatorial(numero):
    ...
    return resultado
def logaritmo_neperiano(n):
    if n <= 0:
        raise ValueError("O número deve ser maior que zero.")

    precisao = 0.00000000000000000001  # Precisão desejada
    termo = (n - 1) / (n + 1)
    resultado = termo
    n_termo = 3
    while abs(termo) > precisao:
        termo *= ((n - 1) / (n + 1)) ** 2
        resultado += termo / n_termo
        n_termo += 2

    return 2 * resultado
    pass
def logaritmo(numero):
    precisao = 1e-10  # Precisão desejada
    termo = (numero - 1) / (numero + 1)
    resultado = termo
    n_termo = 3
    while abs(termo) > precisao:
        termo *= ((numero - 1) / (numero + 1)) ** 2
        resultado += termo / n_termo
        n_termo += 2

    return 2 * resultado
def entradas(tipoentrada,memoria):
    opcoes = ['+','-','*','/','**','r2','r','tg','f','logn','log']
    opcoestrigonometricas = ['s','c','t','sec','cossec','cot']
    menu_entradas = ['1','2','3','4','5','x']
    menu_conversao = ['1','2','x']
    if tipoentrada == 'v1' or tipoentrada == 'v2':
        while True:
            entrada = input(f'Digite o valor{tipoentrada[1]} :')
            try:
                entrada = float(entrada)
                return entrada
            except:
                print('Formatato digitado não suportado ,por favor digite novamente:')
    elif tipoentrada == 'm':
        entrada = input('Digite a opção desejada :')
        validador_entradas = [True for i in menu_entradas if i == entrada]
        try:
        ...
        entrada = input('Digite a opção desejada :')
        validador_entradas = [True for i in menu_conversao if i == entrada]
        try:
                validador_entradas[0]
                return entrada
        except:
        ...
        while True:
            entrada = input('Digite por favor + - * /  f(fatorial) **(potencia) r2(raiz quadrada) r(raiz indice n) tg(Trigonometria) ,log(logaritmo) ou logn(Logaritmo neperiano):')
            validador_entradas = [True for i in opcoes if i == entrada]
            try:
                validador_entradas[0]
                return entrada
            except:
        ...
        while True:
            entrada = input(f'Por favor digite o valor da potencia para o numero {memoria}  :')
            try:
                entrada = float(entrada)
                return entrada
            except:
        ...
        while True:
            if  memoria > 0:
                ...
                print('Não existe raiz de número  negativo , Por favor Veirifique o valor digitado !!!')
                memoria = entradas('v1',0)
    elif tipoentrada == 'r':
        while True:
            entrada = input('Por favor digite o Índice da raiz :')
            try:
                entrada = float(entrada)
                return entrada
            except:
                print('Formatato digitado não suportado ,por favor digite novamente:')
    elif tipoentrada == 'tg1':
        while True:
            entrada = input('Digite o operador que deseja usar SENO = s ,COSSENO = c e TANGENTE = t \n SECANTE = sec ,COSSECANTE = cossec ,COTANGENT = cot :')
            if entrada in opcoestrigonometricas:
                return entrada
    elif tipoentrada == 'tg2':
        while True:
            entrada = input('Digite g para Graus ou r para Radianos : ')
            if entrada == 'g' or entrada == 'r':
                return entrada
            else:
                print('Por favor verifique sua entrada !!!')
    elif tipoentrada == 'f':
        while True:
            if memoria.is_integer() == True and memoria > 0:
                return int(memoria)
            else:
                print('Não existe Fatorial de número decimal ou negativo , Por favor Veirifique o valor digitado !!!')
                memoria = entradas('v1',0)
    elif tipoentrada == 'logn' or tipoentrada == 'log':
        while True:
            if  memoria > 0:
                return memoria
            else:
                print('Não existe logaritmo de número  negativo , Por favor Veirifique o valor digitado !!!')
                memoria = entradas('v1',0)
    elif tipoentrada == 'd':
        entrada = input('Digite enter para fazer outra operção com essa memoria ,n zerar valores e X para menu:')
        return entrada
def formatacao(num):
    if num.is_integer():
        return int(num)
...
def mostra_menu():
    print('\n')
    print(' *************************Calculadora************************************\n')
    print(' 1 - calcular             2 - Equação 2º grau      3 - Conversor de valores')
    print('   + - * /                      x²+x+1                                     ')
    print('   log                                                                   \n')
    print(' 4 - sistema linear   5 - Grafico de equações           X - Sair           ')
def conversor_tempo():
...
def conversor_energia():
    ...
    while True:
        ...
        if d == 'n':
            break
def mostra_menu_conversao():
    def menu_conversao():
        print('\n')
        print(' *******************************Menu Conversão************************************\n')
        print(' 1 - Conversor de Tempo         2 - Conversor de Energia        3 - Ou X para sair  ')
    while True:
        menu_conversao()
        print('\n')
        menu = entradas('mc',0)
